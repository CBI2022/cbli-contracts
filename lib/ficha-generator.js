  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const infoRows = [
        ...
        ['Bedrooms', val(f.bedrooms), 'Garage', val(f.coveredGarage)],
        ['Bathrooms', val(f.bathrooms), 'How many cars can park?', val(f.parkings)],
        ['Guest Toilet', val(f.guestToilet), 'Furnished', val(f.furnitureIncluded)],
        ['Kitchen', val(f.kitchen), 'Storage Room', val(f.storageRoom)],
      ];
      ...
      const docsData = [
        ...
        ['CEE (Energy C.)', f.ceeRating ? 'A - B - C - D - E - F - G  →  ' + f.ceeRating : '—'],
      ];
