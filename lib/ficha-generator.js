  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      try {
        ...
        // Logo is 1920x1080 — scale to fit header (width ~300, height proportional)
        const logoW = 280;
      ...
      // Commission Rate — always show with % and IVA status
      const commRate = f.commissionRate || '';
      if (commRate) {
        const rateNum = commRate.replace('%', '').trim();
        const rateStr = rateNum + '%';
        const ivaStr = (f.ivaIncluded === true || f.ivaIncluded === 'true') ? ' - IVA Included' : ' - IVA not Included';
        doc.fontSize(11).font('Helvetica-Bold').fillColor(RED);
        doc.text('Commission Rate:', margin, y);
        doc.fontSize(11).font('Helvetica-Bold').fillColor(BLACK);
        doc.text(rateStr + ivaStr, margin + 120, y);
        y += 25;
      }
