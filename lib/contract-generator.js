...
function buildCommissionContract(doc, d, t) {
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      ...
      if (selHasPartner) {
        ...
        mixedPara([
          { text: 'On the one hand, ', italic: true },
          { text: `${mrs1} ${sellerName}`, bold: true },
          { text: ' and ', italic: true },
          { text: `${mrs2} ${v(d.seller.partner.name)}`, bold: true },
          { text: `, of ${v(d.seller?.nationality)} nationality, of legal age, married, with ${v(d.seller?.idType)} number `, italic: true },
          { text: v(d.seller?.idNumber), bold: true },
          { text: ' and ', italic: true },
          { text: v(d.seller.partner.idNumber), bold: true },
          { text: `, residing at ${v(d.seller?.address)}. Hereinafter the `, italic: true },
        ...
        mixedPara([
          { text: 'On the one hand, ', italic: true },
          { text: `${mrs1} ${sellerName}`, bold: true },
          { text: `, of ${v(d.seller?.nationality)} nationality, of legal age, with ${v(d.seller?.idType)} number `, italic: true },
          { text: v(d.seller?.idNumber), bold: true },
          { text: `, residing at ${v(d.seller?.address)}. Hereinafter the `, italic: true },
          { text: 'Seller.', bold: true },
        ], { spacingAfter: 6, color });
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      const color = LANG_COLORS[entry.lang] || '#333333';
      mixedPara([
        { text: 'And on the other hand, the company ', italic: true },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
        { text: ' with CIF ', italic: true },
        { text: 'B56281082', bold: true },
        { text: ', with address at Puerto Deportivo Campomanes 59, 03590, Altea, Alicante on behalf of Don ', italic: true },
        { text: 'BRUNO FELIPE', bold: true },
        { text: '.', italic: true },
      ], { spacingAfter: 6, color });
      doc.fillColor('#000000');
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      let dicenSegsT = [
        ...
        { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
        { text: ` has offered and visited with the client ${buyT1} `, italic: true },
        { text: buyerName, bold: true },
        { text: `, of legal age, with nationality ${v(d.buyer?.nationality)}, provided with ${v(d.buyer?.idType)} number: `, italic: true },
        { text: v(d.buyer?.idNumber), bold: true },
      ];
      if (buyHasPartner) {
        const buyT2 = (d.buyer.partner.title === 'Doña' || d.buyer.partner.title === 'Dona') ? 'Mrs' : 'Mr';
        dicenSegsT.push(
          { text: `, and ${buyT2} `, italic: true },
          { text: v(d.buyer.partner.name), bold: true },
          { text: `, of legal age, with nationality ${v(d.buyer?.partner?.nationality || d.buyer?.nationality)}, provided with ${v(d.buyer.partner.idType)} number: `, italic: true },
          { text: v(d.buyer.partner.idNumber), bold: true }
        );
      }
      dicenSegsT.push({ text: `, the following property of the Seller: URBAN: ${v(d.property?.type)}, located at ${v(d.property?.address)}`, italic: true });
      if (hasVal(d.property?.registry)) dicenSegsT.push({ text: ` registered in the ${d.property.registry.trim()} Property Registry`, italic: true });
      if (hasVal(d.property?.finca)) dicenSegsT.push({ text: `, property no. ${d.property.finca.trim()}`, italic: true });
      if (hasVal(d.property?.tomo)) dicenSegsT.push({ text: `, volume ${d.property.tomo.trim()}`, italic: true });
      if (hasVal(d.property?.libro)) dicenSegsT.push({ text: `, book ${d.property.libro.trim()}`, italic: true });
      if (hasVal(d.property?.folio)) dicenSegsT.push({ text: `, folio ${d.property.folio.trim()}`, italic: true });
      if (hasVal(d.property?.catastral)) {
        dicenSegsT.push({ text: ', with only ', italic: true });
        dicenSegsT.push({ text: `cadastral reference: ${d.property.catastral.trim()}`, bold: true });
      }
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const isLast = idx === doc._langEntries.length - 1;
      mixedPara([
        { text: 'The clients mentioned above want to buy through the Real Estate agency ', italic: true },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
        { text: ' the property described in the previous section.', italic: true },
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const isLast = idx === doc._langEntries.length - 1;
      mixedPara([
        { text: 'The agreed price for the sale of said home is ', italic: true },
        { text: `${formatMoney(d.price?.total, entry.lang)}`, bold: true },
        { text: '. The agreed commission of the intermediary is included in the sale price of a total of ', italic: true },
        { text: `${formatMoney(String(commEffective), entry.lang)} + 21% VAT = a total of ${formatMoney(String(commWithIVA), entry.lang)}`, bold: true },
        { text: '.', italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const isLast = idx === doc._langEntries.length - 1;
      mixedPara([
        { text: 'Upon the payment of the 10% deposit as downpayment money into the seller\'s account, the seller will then pay a part of the total agreed commission, which amounts to ', italic: true },
        { text: `${formatMoney(d.commission?.firstPayment, entry.lang)} + 21% VAT = a total of ${formatMoney(String(fpIVA), entry.lang)}`, bold: true },
        { text: '.', italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const isLast = idx === doc._langEntries.length - 1;
      mixedPara([
        { text: 'The remaining commission, which amounts to ', italic: true },
        { text: `${formatMoney(d.commission?.secondPayment, entry.lang)} + 21% VAT = a total of ${formatMoney(String(spIVA), entry.lang)}`, bold: true },
        { text: ', will be paid to ', italic: true },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS SL', bold: true },
        { text: ' either before or on the same day as the signing at the agreed-upon notary.', italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  }
  ...
  bilingual(doc,
    ...
    (t, lang) => 'Both parties read this document, find it in accordance with their will, ratify its content, and sign it in duplicate, for a single purpose, at the aforementioned place and date.'
  );
