...
function buildCommissionContract(doc, d, t) {
  ...
  // Title
  doc.fillColor('#000000');
  writePara(doc, 'RECONOCIMIENTO DE HONORARIOS', { bold: true, fontSize: 14, align: 'center', spacingAfter: 2 });
  writePara(doc, 'COMMISSION AGREEMENT', { bold: true, italic: true, fontSize: 14, align: 'center', spacingAfter: 14 });
  ...
  if (selHasPartner) {
    ...
    writePara(doc, `De una parte, ${selT1} ${sellerName} y ${selT2} ${v(pt.name)}, de nacionalidad ${v(d.seller?.nationality)}, mayores de edad, casados, con ${v(d.seller?.idType)} número ${v(d.seller?.idNumber)} y ${v(pt.idNumber)}, con domicilio en ${v(d.seller?.address)}. En adelante el Vendedor.`, { spacingAfter: 6 });
  } else {
    const selT1 = (d.seller?.title === 'Doña' || d.seller?.title === 'Dona') ? 'DOÑA' : 'DON';
    writePara(doc, `De una parte, ${selT1} ${sellerName}, de nacionalidad ${v(d.seller?.nationality)}, mayor de edad, con ${v(d.seller?.idType)} número ${v(d.seller?.idNumber)}, con domicilio en ${v(d.seller?.address)}. En adelante el Vendedor.`, { spacingAfter: 6 });
  }
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      ...
      let text = `On the one hand, ${mrs1} ${sellerName}`;
      if (selHasPartner) {
        const mrs2 = (d.seller.partner.title === 'Doña' || d.seller.partner.title === 'Dona') ? 'Mrs' : 'Mr';
        text += ` and ${mrs2} ${v(d.seller.partner.name)}, of ${v(d.seller?.nationality)} nationality, of legal age, married, with ${v(d.seller?.idType)} number ${v(d.seller?.idNumber)} and ${v(d.seller.partner.idNumber)}`;
      } else {
        text += `, of ${v(d.seller?.nationality)} nationality, of legal age, with ${v(d.seller?.idType)} number ${v(d.seller?.idNumber)}`;
      }
      text += `, residing at ${v(d.seller?.address)}. Hereinafter the Seller.`;
      writePara(doc, text, { italic: true, spacingAfter: 6 });
      doc.fillColor('#000000');
    });
  }
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      const color = LANG_COLORS[entry.lang] || '#333333';
      doc.fillColor(color);
      writePara(doc, 'And on the other hand, the company COSTA BLANCA LUXURY INVESTMENTS S.L. with CIF B56281082, with address at Puerto Deportivo Campomanes 59, 03590, Altea, Alicante on behalf of Don BRUNO FELIPE.', { italic: true, spacingAfter: 6 });
      doc.fillColor('#000000');
    });
  }
  ...
  let dicenEs = `Que COSTA BLANCA LUXURY INVESTMENTS S.L. ha ofrecido y visitado con la cliente ${buyT1} ${buyerName}, mayor de edad, con nacionalidad ${v(d.buyer?.nationality)}, provista de número de ${v(d.buyer?.idType)}: ${v(d.buyer?.idNumber)}`;
  if (buyHasPartner) {
    const buyT2 = (d.buyer.partner.title === 'Doña' || d.buyer.partner.title === 'Dona') ? 'Doña' : 'Don';
    dicenEs += `, y ${buyT2} ${v(d.buyer.partner.name)}, mayor de edad, con nacionalidad ${v(d.buyer?.partner?.nationality || d.buyer?.nationality)}, provista de número de ${v(d.buyer.partner.idType)}: ${v(d.buyer.partner.idNumber)}`;
  }
  dicenEs += ` la siguiente propiedad de la parte Vendedora: URBANA: ${v(d.property?.type)}, ubicada en ${v(d.property?.address)}`;
  if (hasVal(d.property?.registry)) dicenEs += ` registrada en el Registro de la Propiedad de ${d.property.registry.trim()}`;
  if (hasVal(d.property?.finca)) dicenEs += `, finca nº ${d.property.finca.trim()}`;
  if (hasVal(d.property?.tomo)) dicenEs += `, tomo ${d.property.tomo.trim()}`;
  if (hasVal(d.property?.libro)) dicenEs += `, libro ${d.property.libro.trim()}`;
  if (hasVal(d.property?.folio)) dicenEs += `, folio ${d.property.folio.trim()}`;
  if (hasVal(d.property?.catastral)) dicenEs += `, con única referencia catastral: ${d.property.catastral.trim()}`;
  dicenEs += '.';

  bilingual(doc, dicenEs,
    (t, lang) => {
      let text = `That COSTA BLANCA LUXURY INVESTMENTS S.L. has offered and visited with the client ${buyT1} ${buyerName}, of legal age, with nationality ${v(d.buyer?.nationality)}, provided with ${v(d.buyer?.idType)} number: ${v(d.buyer?.idNumber)}`;
      if (buyHasPartner) {
        const buyT2 = (d.buyer.partner.title === 'Doña' || d.buyer.partner.title === 'Dona') ? 'Mrs' : 'Mr';
        text += `, and ${buyT2} ${v(d.buyer.partner.name)}, of legal age, with nationality ${v(d.buyer?.partner?.nationality || d.buyer?.nationality)}, provided with ${v(d.buyer.partner.idType)} number: ${v(d.buyer.partner.idNumber)}`;
      }
  ...
  bilingual(doc,
    'Los clientes arriba mencionados quieren comprar por mediación de la agencia Inmobiliaria COSTA BLANCA LUXURY INVESTMENTS S.L. la propiedad descrita en el apartado anterior.',
    (t, lang) => 'The clients mentioned above want to buy through the Real Estate agency COSTA BLANCA LUXURY INVESTMENTS S.L. the property described in the previous section.'
  );
  ...
  bilingual(doc,
    `El precio pactado para la venta de dicha vivienda es de ${formatMoney(d.price?.total)}. La comisión pactada de los dos intermediarios está incluida en el precio de venta de un total de ${formatMoney(String(commEffective))} + 21% IVA = un total de ${formatMoney(String(commWithIVA))}.`,
    (t, lang) => `The agreed price for the sale of said home is ${formatMoney(d.price?.total, lang)}. The agreed commission of the two intermediaries is included in the sale price of a total of ${formatMoney(String(commEffective), lang)} + 21% VAT = a total of ${formatMoney(String(commWithIVA), lang)}.`
  );
  ...
  const fpIVA = fp * 1.21;
  bilingual(doc,
    `Al abonarse el depósito del 10% en concepto de arras en la cuenta del vendedor, este abonará el 50% del total de la comisión pactada, que sería ${formatMoney(d.commission?.firstPayment)} + 21% IVA = un total de ${formatMoney(String(fpIVA))}.`,
    (t, lang) => `Upon the payment of the 10% deposit as downpayment money into the seller's account, the seller will then pay 50% of the total agreed commission, which amounts to ${formatMoney(d.commission?.firstPayment, lang)} + 21% VAT = a total of ${formatMoney(String(fpIVA), lang)}.`
  );
  ...
  const spIVA = sp * 1.21;
  bilingual(doc,
    `El restante de la comisión, (50%) del total que asciende a ${formatMoney(d.commission?.secondPayment)} + 21% IVA = un total de ${formatMoney(String(spIVA))} será pagada a la inmobiliaria COSTA BLANCA LUXURY INVESTMENTS SL antes o el mismo dia de la firma en la notaría acordada.`,
    (t, lang) => `The remaining commission, which constitutes (50%) of the total amounts to ${formatMoney(d.commission?.secondPayment, lang)} + 21% VAT = a total of ${formatMoney(String(spIVA), lang)}, will be paid to COSTA BLANCA LUXURY INVESTMENTS SL either before or on the same day as the signing at the agreed-upon notary.`
  );
