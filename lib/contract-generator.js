...
const _esFmt = new Intl.NumberFormat('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2, useGrouping: true });

function formatMoney(amount, lang) {
  if (!amount) return '___';
  const num = Number(amount);
  const formatted = _esFmt.format(num);
...
function buildReservationContract(doc, d, t) {
  ...
  // CBLI bank details — always use fixed CBLI Sabadell account for reserva deposits
  writePara(doc, `IBAN: ${CBLI_BANK.iban}`, { bold: true, indent: 36, spacingAfter: 1 });
  writePara(doc, `Banco: ${CBLI_BANK.bankName}`, { indent: 36, spacingAfter: 1 });
  writePara(doc, `Beneficiario: ${CBLI_BANK.beneficiary}`, { bold: true, indent: 36, spacingAfter: 1 });
...
function buildCommissionContract(doc, d, t) {
  ...
  // Helper: write segments with mixed bold/normal in one paragraph
  const mixedPara = (segments, opts = {}) => {
    const { align, spacingAfter, indent, color } = opts;
    const options = { align: align || 'left' };
    if (indent) options.indent = indent;
    segments.forEach((seg, idx) => {
      if (seg.bold) {
        doc.font('Helvetica-Bold');
      } else if (seg.italic && seg.bold) {
        doc.font('Helvetica-BoldOblique');
      } else if (seg.italic) {
        doc.font('Helvetica-Oblique');
      } else {
    ...
    if (color) doc.fillColor('#000000');
    doc.font('Helvetica');
    doc.fontSize(11);
    const sp = spacingAfter !== undefined ? spacingAfter : 10;
    if (sp) doc.moveDown(sp / 12);
  };
  ...
  // Title — Spanish only, black, underlined, NO translation
  doc.fillColor('#000000');
  writePara(doc, 'RECONOCIMIENTO DE HONORARIOS', { bold: true, underline: true, fontSize: 14, align: 'center', spacingAfter: 14 });

  // COMPARECEN
  writePara(doc, 'COMPARECEN', { bold: true, underline: true, align: 'center', spacingAfter: 10 });

  // --- Seller (ES) with bold names, doc numbers, "Vendedor" ---
  const selT1 = (d.seller?.title === 'Doña' || d.seller?.title === 'Dona') ? 'DOÑA' : 'DON';
  if (selHasPartner) {
    const pt = d.seller.partner;
    const selT2 = (pt.title === 'Doña' || pt.title === 'Dona') ? 'DOÑA' : 'DON';
    mixedPara([
      { text: 'De una parte, ' },
      { text: `${selT1} ${sellerName}`, bold: true },
      { text: ' y ' },
      { text: `${selT2} ${v(pt.name)}`, bold: true },
      { text: `, de nacionalidad ${v(d.seller?.nationality)}, mayores de edad, casados, con ${v(d.seller?.idType)} número ` },
      { text: v(d.seller?.idNumber), bold: true },
      { text: ' y ' },
      { text: v(pt.idNumber), bold: true },
      { text: `, con domicilio en ${v(d.seller?.address)}. En adelante el ` },
    ...
    mixedPara([
      { text: 'De una parte, ' },
      { text: `${selT1} ${sellerName}`, bold: true },
      { text: `, de nacionalidad ${v(d.seller?.nationality)}, mayor de edad, con ${v(d.seller?.idType)} número ` },
      { text: v(d.seller?.idNumber), bold: true },
      { text: `, con domicilio en ${v(d.seller?.address)}. En adelante el ` },
      { text: 'Vendedor.', bold: true },
    ], { spacingAfter: 6 });
  ...
  // --- Seller TRANSLATION right after seller (before CBLI) ---
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      const color = LANG_COLORS[entry.lang] || '#333333';
      const tr = entry.t;
      const mrs1 = (d.seller?.title === 'Doña' || d.seller?.title === 'Dona') ? 'Mrs' : 'Mr';
      const natTranslated = NATIONALITY_TRANSLATIONS[entry.lang]?.[v(d.seller?.nationality).toLowerCase()] || v(d.seller?.nationality);
      if (selHasPartner) {
        const mrs2 = (d.seller.partner.title === 'Doña' || d.seller.partner.title === 'Dona') ? 'Mrs' : 'Mr';
        mixedPara([
          { text: `${tr.comm_on_one_hand} `, italic: true },
          { text: `${mrs1} ${sellerName}`, bold: true },
          { text: ` ${tr.comm_and} `, italic: true },
          { text: `${mrs2} ${v(d.seller.partner.name)}`, bold: true },
          { text: `, ${tr.comm_of_nationality} ${natTranslated}, ${tr.comm_of_legal_age}, ${tr.comm_married}, ${tr.comm_with} ${v(d.seller?.idType)} ${tr.comm_number} `, italic: true },
          { text: v(d.seller?.idNumber), bold: true },
          { text: ` ${tr.comm_and} `, italic: true },
          { text: v(d.seller.partner.idNumber), bold: true },
          { text: `, ${tr.comm_residing_at} ${v(d.seller?.address)}. ${tr.comm_hereinafter_seller}`, italic: true },
        ], { spacingAfter: 6, color });
      } else {
        mixedPara([
          { text: `${tr.comm_on_one_hand} `, italic: true },
          { text: `${mrs1} ${sellerName}`, bold: true },
          { text: `, ${tr.comm_of_nationality} ${natTranslated}, ${tr.comm_of_legal_age}, ${tr.comm_with} ${v(d.seller?.idType)} ${tr.comm_number} `, italic: true },
          { text: v(d.seller?.idNumber), bold: true },
          { text: `, ${tr.comm_residing_at} ${v(d.seller?.address)}. ${tr.comm_hereinafter_seller}`, italic: true },
        ], { spacingAfter: 6, color });
      }
  ...
  // --- CBLI (ES) with bold company name and Bruno ---
  mixedPara([
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      ...
      const tr = entry.t;
      mixedPara([
        { text: `${tr.comm_other_hand_company} `, italic: true },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
        { text: ` ${tr.comm_with_cif} `, italic: true },
        { text: 'B56281082', bold: true },
        { text: `, ${tr.comm_with_address_at} Puerto Deportivo Campomanes 59, 03590, Altea, Alicante ${tr.comm_on_behalf_of} `, italic: true },
        { text: 'BRUNO FELIPE', bold: true },
        { text: '.', italic: true },
      ], { spacingAfter: 6, color });
      doc.fillColor('#000000');
  ...
  // --- DICEN paragraph (ES) with bold names, doc numbers, catastral ---
  const buyT1 = (d.buyer?.title === 'Doña' || d.buyer?.title === 'Dona') ? 'Doña' : 'Don';
  let dicenSegsEs = [
    { text: 'Que ' },
    { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
    { text: ` ha ofrecido y visitado con la cliente ${buyT1} ` },
    { text: buyerName, bold: true },
    { text: `, mayor de edad, con nacionalidad ${v(d.buyer?.nationality)}, provista de número de ${v(d.buyer?.idType)}: ` },
    { text: v(d.buyer?.idNumber), bold: true },
  ];
  if (buyHasPartner) {
    const buyT2 = (d.buyer.partner.title === 'Doña' || d.buyer.partner.title === 'Dona') ? 'Doña' : 'Don';
    dicenSegsEs.push(
      { text: `, y ${buyT2} ` },
      { text: v(d.buyer.partner.name), bold: true },
      { text: `, mayor de edad, con nacionalidad ${v(d.buyer?.partner?.nationality || d.buyer?.nationality)}, provista de número de ${v(d.buyer.partner.idType)}: ` },
      { text: v(d.buyer.partner.idNumber), bold: true }
    );
  }
  dicenSegsEs.push({ text: ` la siguiente propiedad de la parte Vendedora: URBANA: ${v(d.property?.type)}, ubicada en ${v(d.property?.address)}` });
  if (hasVal(d.property?.registry)) dicenSegsEs.push({ text: ` registrada en el Registro de la Propiedad de ${d.property.registry.trim()}` });
  if (hasVal(d.property?.finca)) dicenSegsEs.push({ text: `, finca nº ${d.property.finca.trim()}` });
  if (hasVal(d.property?.tomo)) dicenSegsEs.push({ text: `, tomo ${d.property.tomo.trim()}` });
  if (hasVal(d.property?.libro)) dicenSegsEs.push({ text: `, libro ${d.property.libro.trim()}` });
  if (hasVal(d.property?.folio)) dicenSegsEs.push({ text: `, folio ${d.property.folio.trim()}` });
  if (hasVal(d.property?.catastral)) {
    dicenSegsEs.push({ text: ', con única ' });
    dicenSegsEs.push({ text: `referencia catastral: ${d.property.catastral.trim()}`, bold: true });
  }
  dicenSegsEs.push({ text: '.' });
  mixedPara(dicenSegsEs, { spacingAfter: doc._spanishOnly ? 12 : 4 });

  // DICEN paragraph (translations)
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      const color = LANG_COLORS[entry.lang] || '#333333';
      const tr = entry.t;
      const isLast = idx === doc._langEntries.length - 1;
      const buyNatTranslated = NATIONALITY_TRANSLATIONS[entry.lang]?.[v(d.buyer?.nationality).toLowerCase()] || v(d.buyer?.nationality);
      let dicenSegsT = [
        { text: `${tr.comm_has_offered} `, italic: true },
        { text: `${buyT1} ${buyerName}`, bold: true },
        { text: `, ${tr.comm_of_legal_age_with_nat} ${buyNatTranslated}, ${tr.comm_provided_with} ${v(d.buyer?.idType)} ${tr.comm_number}: `, italic: true },
        { text: v(d.buyer?.idNumber), bold: true },
      ];
      if (buyHasPartner) {
        const buyT2 = (d.buyer.partner.title === 'Doña' || d.buyer.partner.title === 'Dona') ? 'Mrs' : 'Mr';
        const buyPartNat = NATIONALITY_TRANSLATIONS[entry.lang]?.[v(d.buyer?.partner?.nationality || d.buyer?.nationality).toLowerCase()] || v(d.buyer?.partner?.nationality || d.buyer?.nationality);
        dicenSegsT.push(
          { text: `, ${tr.comm_and} ${buyT2} `, italic: true },
          { text: v(d.buyer.partner.name), bold: true },
          { text: `, ${tr.comm_of_legal_age_with_nat} ${buyPartNat}, ${tr.comm_provided_with} ${v(d.buyer.partner.idType)} ${tr.comm_number}: `, italic: true },
          { text: v(d.buyer.partner.idNumber), bold: true }
        );
      }
      dicenSegsT.push({ text: `, ${tr.comm_following_property} ${v(d.property?.type)}, ${tr.comm_located_at} ${v(d.property?.address)}`, italic: true });
      if (hasVal(d.property?.registry)) dicenSegsT.push({ text: ` ${tr.comm_registered_in} ${d.property.registry.trim()} ${tr.comm_property_registry}`, italic: true });
      if (hasVal(d.property?.finca)) dicenSegsT.push({ text: `, ${tr.comm_property_no} ${d.property.finca.trim()}`, italic: true });
      if (hasVal(d.property?.tomo)) dicenSegsT.push({ text: `, ${tr.comm_volume} ${d.property.tomo.trim()}`, italic: true });
      if (hasVal(d.property?.libro)) dicenSegsT.push({ text: `, ${tr.comm_book} ${d.property.libro.trim()}`, italic: true });
      if (hasVal(d.property?.folio)) dicenSegsT.push({ text: `, ${tr.comm_folio} ${d.property.folio.trim()}`, italic: true });
      if (hasVal(d.property?.catastral)) {
        dicenSegsT.push({ text: `, ${tr.comm_cadastral_ref} `, italic: true });
        dicenSegsT.push({ text: d.property.catastral.trim(), bold: true });
      }
      dicenSegsT.push({ text: '.', italic: true });
      mixedPara(dicenSegsT, { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  }
  ...
  ], { spacingAfter: doc._spanishOnly ? 12 : 4 });
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      const color = LANG_COLORS[entry.lang] || '#333333';
      const tr = entry.t;
      const isLast = idx === doc._langEntries.length - 1;
      mixedPara([
        { text: `${tr.comm_clients_want} `, italic: true },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
        { text: ` ${tr.comm_property_described}`, italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  }

  // Price + commission paragraph — "del intermediario" (not "de los dos intermediarios")
  const commEffective = commAfterDiscount > 0 ? commAfterDiscount : commNet;
  const commWithIVA = commEffective * 1.21;
  mixedPara([
    ...
    { text: `${formatMoney(String(commEffective))} + 21% IVA = un total de ${formatMoney(String(commWithIVA))}`, bold: true },
    { text: '.' },
  ], { spacingAfter: doc._spanishOnly ? 12 : 4 });
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const tr = entry.t;
      mixedPara([
        { text: `${tr.comm_agreed_price} `, italic: true },
        { text: `${formatMoney(d.price?.total, entry.lang)}`, bold: true },
        { text: `. ${tr.comm_agreed_commission} `, italic: true },
        { text: `${formatMoney(String(commEffective), entry.lang)} + 21% ${tr.comm_vat_total} ${formatMoney(String(commWithIVA), entry.lang)}`, bold: true },
        { text: '.', italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  ...
  // First payment at arras — Bruno's exact text, no "50%"
  ...
  mixedPara([
    ...
    { text: `${formatMoney(d.commission?.firstPayment)} + 21% IVA = un total de ${formatMoney(String(fpIVA))}`, bold: true },
    { text: '.' },
  ], { spacingAfter: doc._spanishOnly ? 12 : 4 });
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const tr2 = entry.t;
      mixedPara([
        { text: `${tr2.comm_first_payment} `, italic: true },
        { text: `${formatMoney(d.commission?.firstPayment, entry.lang)} + 21% ${tr2.comm_vat_total} ${formatMoney(String(fpIVA), entry.lang)}`, bold: true },
        { text: '.', italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  ...
  // Second payment at notary — no "50%"
  const sp = parseFloat(d.commission?.secondPayment) || 0;
  const spIVA = sp * 1.21;
  mixedPara([
    { text: 'El restante de la comisión, que asciende a ' },
    { text: `${formatMoney(d.commission?.secondPayment)} + 21% IVA = un total de ${formatMoney(String(spIVA))}`, bold: true },
  ...
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry, idx) => {
      ...
      const tr3 = entry.t;
      mixedPara([
        { text: `${tr3.comm_second_payment} `, italic: true },
        { text: `${formatMoney(d.commission?.secondPayment, entry.lang)} + 21% ${tr3.comm_vat_total} ${formatMoney(String(spIVA), entry.lang)}`, bold: true },
        { text: `, ${tr3.comm_paid_to} `, italic: true },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS SL', bold: true },
        { text: ` ${tr3.comm_before_notary}`, italic: true },
      ], { spacingAfter: isLast ? 14 : 4, color });
      doc.fillColor('#000000');
    });
  }
  ...
  // Bank details (ES)
  writePara(doc, `Cuenta: ${v(d.bank?.iban)}`, { spacingAfter: 1 });
  writePara(doc, 'SWIFT: CCRIES2A045', { spacingAfter: 1 });
  writePara(doc, `Banco: ${(v(d.bank?.bankName)).toUpperCase()}`, { spacingAfter: 1 });
  writePara(doc, `Beneficiario: ${v(d.bank?.beneficiary)}`, { spacingAfter: 1 });
  writePara(doc, 'Concepto: Comision', { spacingAfter: 10 });

  // Bank details (translations)
  if (!doc._spanishOnly && doc._langEntries?.length > 0) {
    doc._langEntries.forEach((entry) => {
      const color = LANG_COLORS[entry.lang] || '#333333';
      doc.fillColor(color);
      writePara(doc, `Account: ${v(d.bank?.iban)}`, { italic: true, spacingAfter: 1 });
      writePara(doc, 'SWIFT: CCRIES2A045', { italic: true, spacingAfter: 1 });
      writePara(doc, `Bank: ${(v(d.bank?.bankName)).toUpperCase()}`, { italic: true, spacingAfter: 1 });
      writePara(doc, `Beneficiary: ${v(d.bank?.beneficiary)}`, { italic: true, spacingAfter: 1 });
      writePara(doc, 'Concept: Commission', { italic: true, spacingAfter: 10 });
      doc.fillColor('#000000');
    });
  }
  ...
  bilingual(doc,
    ...
    (t, lang) => t.comm_closing
  );

  // Signatures — CBLI Spanish only, seller/buyer bilingual
  doc.moveDown(3);
  setFont(doc, true, false);
  const pageWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const colWidth = pageWidth / 3;
  ...
  doc.fillColor('#000000');
  if (doc._spanishOnly) {
    doc.text('VENDEDOR', startX, sigY, { width: colWidth, align: 'center', continued: false });
    doc.text('COMPRADOR', startX + colWidth, sigY, { width: colWidth, align: 'center', continued: false });
  } else {
    doc.text('VENDEDOR / SELLER', startX, sigY, { width: colWidth, align: 'center', continued: false });
    doc.text('COMPRADOR / BUYER', startX + colWidth, sigY, { width: colWidth, align: 'center', continued: false });
  }
  doc.text('COSTA BLANCA LUXURY', startX + colWidth * 2, sigY, { width: colWidth, align: 'center', continued: false });
  doc.text('INVESTMENTS S.L.', startX + colWidth * 2, sigY + 12, { width: colWidth, align: 'center', continued: false });
  ...
  doc.fontSize(9);
  const sellerFullName = d.seller?.name || '';
  const sellerPartnerName = selHasPartner ? d.seller.partner.name : '';
  const buyerFullName = d.buyer?.name || '';
  const buyerPartnerName = buyHasPartner ? d.buyer.partner.name : '';
