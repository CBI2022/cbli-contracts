...
function formatDate(dateStr, lang) {
  ...
  if (lang === 'es') return `${day} de ${month} de ${year}`;
  if (lang === 'en') return `${day} of ${month} of ${year}`;
  if (lang === 'nl') return `${day} ${month} ${year}`;
...
function buildArrasContract(doc, d, t) {
  ...
  // Header image
  if (doc._headerImage) {
    doc.image(doc._headerImage, 60, 30, { width: 475 });
    doc.moveDown(5);
  }

  // Title
  writePara(doc, 'CONTRATO DE ARRAS', { bold: true, fontSize: 14, align: 'center', spacingAfter: doc._spanishOnly ? 14 : 2 });
  if (!doc._spanishOnly && doc._langEntries) {
    doc._langEntries.forEach((entry, idx) => {
      const isLast = idx === doc._langEntries.length - 1;
      doc.fillColor(LANG_COLORS[entry.lang] || '#333333');
      writePara(doc, entry.t.arras_title, { bold: true, italic: true, fontSize: 14, align: 'center', spacingAfter: isLast ? 14 : 2 });
      doc.fillColor('#000000');
    });
  }
  ...
  // Date & place
  bilingual(doc,
    `En ${v(d.city)} a ${formatDate(d.date, 'es')}`,
    (t, lang) => `In ${v(d.city)}, ${formatDate(d.date, lang)}`
  );

  // REUNIDOS
  writePara(doc, 'REUNIDOS', { bold: true, underline: true, align: 'center', spacingAfter: doc._spanishOnly ? 10 : 2 });
  if (!doc._spanishOnly && doc._langEntries) {
    doc._langEntries.forEach((entry, idx) => {
      const isLast = idx === doc._langEntries.length - 1;
      doc.fillColor(LANG_COLORS[entry.lang] || '#333333');
      writePara(doc, entry.t.reunidos, { bold: true, italic: true, underline: true, align: 'center', spacingAfter: isLast ? 10 : 2 });
      doc.fillColor('#000000');
    });
  }
  ...
  // EXPONEN
  writePara(doc, 'EXPONEN', { bold: true, underline: true, align: 'center', spacingAfter: doc._spanishOnly ? 10 : 2 });
  if (!doc._spanishOnly && doc._langEntries) {
    doc._langEntries.forEach((entry, idx) => {
      const isLast = idx === doc._langEntries.length - 1;
      doc.fillColor(LANG_COLORS[entry.lang] || '#333333');
      writePara(doc, entry.t.declare, { bold: true, italic: true, underline: true, align: 'center', spacingAfter: isLast ? 10 : 2 });
      doc.fillColor('#000000');
    });
  }
  ...
  writePara(doc, `VIVIENDA: ${v(d.property?.type)}, Ref. ${v(d.property?.ref)}, ubicada en ${v(d.property?.address)}, registrada en el Registro de la Propiedad de ${v(d.property?.registry)}, finca no ${v(d.property?.finca)}, tomo ${v(d.property?.tomo)}, libro ${v(d.property?.libro)}, folio ${v(d.property?.folio)} con unica referencia catastral: ${v(d.property?.catastral)}.`, { bold: true, spacingAfter: 10 });

  // Extra properties (garage/storage) — Spanish only
  if (d.extraProperties) {
    d.extraProperties.forEach(ep => {
      if (ep.enabled) {
        writePara(doc, `${ep.type.toUpperCase()}: referencia catastral ${v(ep.catastral)}, finca no ${v(ep.finca)}, tomo ${v(ep.tomo)}, libro ${v(ep.libro)}, folio ${v(ep.folio)}.`, { bold: true, spacingAfter: 10 });
      }
    });
  }
  ...
  // ESTIPULACIONES
  writePara(doc, 'ESTIPULACIONES', { bold: true, underline: true, align: 'center', spacingAfter: doc._spanishOnly ? 10 : 2 });
  if (!doc._spanishOnly && doc._langEntries) {
    doc._langEntries.forEach((entry, idx) => {
      const isLast = idx === doc._langEntries.length - 1;
      doc.fillColor(LANG_COLORS[entry.lang] || '#333333');
  ...
  // Signatures — seller left, buyer right, same line
  doc.moveDown(3);
  ...
  doc.moveDown(18);
  const cbliY = doc.y;
  const cbliLineX = startX + (pageWidth - lineLen) / 2;
  doc.fontSize(11).text('COSTA BLANCA LUXURY INVESTMENTS SL', startX, cbliY, { width: pageWidth, align: 'center' });
  doc.moveTo(cbliLineX, cbliY + 50).lineTo(cbliLineX + lineLen, cbliY + 50).stroke();
  doc.y = cbliY + 60;
  doc.moveDown(3);
}
...
function buildReservationContract(doc, d, t) {
  ...
  // Property owners
  bilingual(doc,
    `Que ${sellerTitle} ${sellerName}, son propietarios de la totalidad de la propiedad abajo detallada:`,
    (t, lang) => `${sellerTitleTrans} ${sellerName}, ${t.res_owners}`
  );

  // Property details
  writePara(doc, `VIVIENDA: ${v(d.property?.type)}, Ref. ${v(d.property?.ref)}, ubicada en ${v(d.property?.address)}, registrada en el Registro de la Propiedad de ${v(d.property?.registry)}, finca no ${v(d.property?.finca)}, tomo ${v(d.property?.tomo)}, libro ${v(d.property?.libro)}, folio ${v(d.property?.folio)} con unica referencia catastral: ${v(d.property?.catastral)}.`, { bold: true, spacingAfter: 10 });

  // Extra properties (garage/storage)
  if (d.extraProperties) {
    d.extraProperties.forEach(ep => {
      if (ep.enabled) {
        writePara(doc, `${ep.type.toUpperCase()}: referencia catastral ${v(ep.catastral)}, finca no ${v(ep.finca)}, tomo ${v(ep.tomo)}, libro ${v(ep.libro)}, folio ${v(ep.folio)}.`, { bold: true, spacingAfter: 10 });
  ...
  // CBLI bank details
  writePara(doc, 'IBAN: ES84 0081 1025 4100 0173 3079', { bold: true, indent: 36, spacingAfter: 1 });
  writePara(doc, 'BIC: BSABESBBXXX', { bold: true, indent: 36, spacingAfter: 1 });
  writePara(doc, 'Titular: COSTA BLANCA LUXURY INVESTMENTS SL', { bold: true, indent: 36, spacingAfter: 1 });
