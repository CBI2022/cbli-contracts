...
export async function generateContractDocx(formData) {
  ...
  if (d.type === 'arras') {
    ...
    // Bank details — bold, Spanish only (no translations)
    addPara('Cuenta: ' + bankIban, { bold: true, spacing: 80 });
    addPara('SWIFT: CCRIES2A045', { bold: true, spacing: 80 });
    addPara('Banco: ' + bankName.toUpperCase(), { bold: true, spacing: 80 });
    addPara('Beneficiario: ' + bankBeneficiary, { bold: true, spacing: 80 });
