...
export async function generateContractDocx(formData) {
  ...
  const bankIban = d.type === 'reserva' ? CBLI_BANK.iban : (d.bank?.iban?.trim() || CBLI_BANK.iban);
  const bankName = d.type === 'reserva' ? CBLI_BANK.bankName : (d.bank?.bankName?.trim() || CBLI_BANK.bankName);
  const bankBeneficiary = d.type === 'reserva' ? CBLI_BANK.beneficiary : (d.bank?.beneficiary?.trim() || CBLI_BANK.beneficiary);
