...
export async function generateContractDocx(formData) {
  ...
  if (d.type === 'commission') {
    // Two-line title for commission
    paragraphs.push(new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 80, line: 240, lineRule: 'auto' },
      children: [new TextRun({ text: 'RECONOCIMIENTO DE HONORARIOS', bold: true, size: 28, color: BRAND.black, font: FONT })],
    }));
    paragraphs.push(new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 200, line: 240, lineRule: 'auto' },
      children: [new TextRun({ text: 'COMMISSION AGREEMENT', bold: true, italic: true, size: 28, color: BRAND.black, font: FONT })],
    }));
  } else {
  ...
  if (d.type === 'arras') {
    ...
    addBilingual(sellerSegsEs, null, 200);
    ...
    // Seller paragraph (translations)
    if (isMultiLang && langEntries.length > 0) {
      const sellerTransFn = (trans, lang) => {
      ...
      addBilingual('', sellerTransFn, 200);
    }

    // CBLI paragraph (translations)
    if (isMultiLang && langEntries.length > 0) {
      addBilingual('',
    ...
    addBilingual(
      [
        ...
        { text: '. La comisión pactada de los dos intermediarios está incluida en el precio de venta de un total de ' },
      ...
      (trans, lang) => [
        ...
        { text: '. The agreed commission of the two intermediaries is included in the sale price of a total of ' },
    ...
    addBilingual(
      [
        { text: 'Al abonarse el depósito del 10% en concepto de arras en la cuenta del vendedor, este abonará el 50% del total de la comisión pactada, que sería ' },
      ...
      (trans, lang) => [
        { text: `Upon the payment of the 10% deposit as downpayment money into the seller's account, the seller will then pay 50% of the total agreed commission, which amounts to ` },
    ...
    addBilingual(
      [
        { text: 'El restante de la comisión, (50%) del total que asciende a ' },
        { text: formatMoneyFull(d.commission?.secondPayment), bold: true },
        { text: ' + 21% IVA = un total de ' },
        { text: formatMoneyFull(String(spIVA)), bold: true },
        { text: ' será pagada a la inmobiliaria COSTA BLANCA LUXURY INVESTMENTS SL antes o el mismo dia de la firma en la notaría acordada.' },
      ],
      (trans, lang) => [
        { text: 'The remaining commission, which constitutes (50%) of the total amounts to ' },
        { text: formatMoneyFull(d.commission?.secondPayment, lang), bold: true },
        { text: ' + 21% VAT = a total of ' },
        { text: formatMoneyFull(String(spIVA), lang), bold: true },
        { text: ', will be paid to COSTA BLANCA LUXURY INVESTMENTS SL either before or on the same day as the signing at the agreed-upon notary.' },
      ],
      200
    );

    // Bank details (ES)
    addPara('Cuenta: ' + bankIban, { spacing: 80 });
    addPara('SWIFT: CCRIES2A045', { spacing: 80 });
    addPara('Banco: ' + bankName.toUpperCase(), { spacing: 80 });
    addPara('Beneficiario: ' + bankBeneficiary, { spacing: 80 });
    addPara('Concepto: Comision', { spacing: 200 });

    // Bank details (translations)
    if (isMultiLang && langEntries.length > 0) {
      addBilingual('', (trans, lang) => `Account: ${bankIban}`, 80);
      addBilingual('', (trans, lang) => `SWIFT: CCRIES2A045`, 80);
      addBilingual('', (trans, lang) => `Bank: ${bankName.toUpperCase()}`, 80);
      addBilingual('', (trans, lang) => `Beneficiary: ${bankBeneficiary}`, 80);
      addBilingual('', (trans, lang) => `Concept: Commission`, 200);
    }
