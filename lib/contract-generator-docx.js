...
export async function generateContractDocx(formData) {
  ...
  // ---- FIXED addPara: uses TextRun children for proper formatting ----
  const addPara = (text, opts = {}) => {
    const { bold = false, italic = false, align = 'left', fontSize = 22, spacing = 200, color = null } = opts;
  ...
  // ---- addBilingual: Spanish primary + colored translations ----
  ...
  // Date and location (top left, above title)
  addPara(`${formatDate(d.date, 'es')}, en ${v(d.city)}`, { align: 'left', spacing: 200 });

  // Contract title (Spanish + translations) — underlined
  ...
  if (isMultiLang) {
    langEntries.forEach((entry) => {
      const titleTrans = d.type === 'arras' ? entry.t.arras_title : entry.t.reservation_title;
      paragraphs.push(new Paragraph({
        alignment: AlignmentType.CENTER,
  ...
  // ========== REUNIDOS ==========
  addPara(esT.reunidos, { bold: true, align: 'center', spacing: 100, color: BRAND.gold, fontSize: 24 });
  if (isMultiLang) {
    langEntries.forEach((entry) => {
      addPara(entry.t.reunidos, { bold: true, italic: true, align: 'center', spacing: 60, color: LANG_COLORS[entry.lang] });
    });
  }

  // Helper: build NIE fragment only if NIE is filled in
  const nieFragEs = (nie) => nie && nie.trim() && nie.trim() !== '___' ? ` y NIE número ${nie.trim()},` : ',';
  const nieFragTrans = (nie, trans) => nie && nie.trim() && nie.trim() !== '___' ? ` ${trans.and_nie} ${nie.trim()},` : ',';

  // Helper: build person identification string (ES)
  const personIdEs = (title, name, nat, idType, idNumber, nie) =>
    `${title} ${name}, mayor de edad, de nacionalidad ${v(nat)}, provisto de ${v(idType)} vigente numero ${v(idNumber)}${nieFragEs(nie)}`;
  const personIdTrans = (title, name, nat, idType, idNumber, nie, trans, lang) =>
    `${title} ${name}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(nat, lang)}, ${trans.with_valid} ${v(idType)} ${trans.number} ${v(idNumber)}${nieFragTrans(nie, trans)}`;

  // Seller identification (combined with partner if applicable)
  ...
  if (d.seller?.hasPartner && d.seller?.partner?.name) {
    ...
    const sellerTextEs = `${esT.on_one_part}, ${sellerTitle} ${sellerName} y ${spTitle} ${v(sp.name)}, mayores de edad, con nacionalidad ${v(d.seller?.nationality)} e ${v(sp.nationality)}, provistos de ${v(d.seller?.idType)} vigente numero ${v(d.seller?.idNumber)} y ${v(sp.idType)} numero ${v(sp.idNumber)}${nieFragEs(d.seller?.nie)} con domicilio a efectos de notificaciones ${v(d.seller?.address)}, en adelante ${esT.the_seller},`;
    addBilingual(
      sellerTextEs,
      (trans, lang) => `${trans.on_one_part}, ${sellerTitle} ${sellerName} and ${spTitle} ${v(sp.name)}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(d.seller?.nationality, lang)} and ${translateNat(sp.nationality, lang)}, ${trans.with_valid} ${v(d.seller?.idType)} ${trans.number} ${v(d.seller?.idNumber)} and ${v(sp.idType)} ${trans.number} ${v(sp.idNumber)}${nieFragTrans(d.seller?.nie, trans)} ${trans.with_address} ${v(d.seller?.address)}, ${trans.hereinafter} ${trans.the_seller},`,
      200
    );
  } else {
    const sellerTextEs = `${esT.on_one_part}, ${personIdEs(sellerTitle, sellerName, d.seller?.nationality, d.seller?.idType, d.seller?.idNumber, d.seller?.nie)} con domicilio a efectos de notificaciones ${v(d.seller?.address)}, en adelante ${esT.the_seller},`;
    addBilingual(
      sellerTextEs,
      (trans, lang) => `${trans.on_one_part}, ${personIdTrans(sellerTitle, sellerName, d.seller?.nationality, d.seller?.idType, d.seller?.idNumber, d.seller?.nie, trans, lang)} ${trans.with_address} ${v(d.seller?.address)}, ${trans.hereinafter} ${trans.the_seller},`,
      200
    );
  }

  // Buyer identification (combined with partner in same paragraph)
  ...
  if (d.buyer?.hasPartner && d.buyer?.partner?.name) {
    ...
    const buyerTextEs = `${esT.and_other_part}, ${buyerTitle} ${buyerName} y ${bpTitle} ${v(bp.name)}, mayores de edad, con nacionalidad ${v(d.buyer?.nationality)} e ${v(bp.nationality)}, provistos de ${v(d.buyer?.idType)} vigente numero ${v(d.buyer?.idNumber)} y ${v(bp.idType)} numero ${v(bp.idNumber)}${nieFragEs(d.buyer?.nie)} con domicilio a efectos de notificaciones ${v(d.buyer?.address)}, en adelante ${esT.the_buyer},`;
    addBilingual(
      buyerTextEs,
      (trans, lang) => `${trans.and_other_part}, ${buyerTitle} ${buyerName} and ${bpTitle} ${v(bp.name)}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(d.buyer?.nationality, lang)} and ${translateNat(bp.nationality, lang)}, ${trans.with_valid} ${v(d.buyer?.idType)} ${trans.number} ${v(d.buyer?.idNumber)} and ${v(bp.idType)} ${trans.number} ${v(bp.idNumber)}${nieFragTrans(d.buyer?.nie, trans)} ${trans.with_address} ${v(d.buyer?.address)}, ${trans.hereinafter} ${trans.the_buyer},`,
      200
    );
  } else {
    const buyerTextEs = `${esT.and_other_part}, ${personIdEs(buyerTitle, buyerName, d.buyer?.nationality, d.buyer?.idType, d.buyer?.idNumber, d.buyer?.nie)} con domicilio a efectos de notificaciones ${v(d.buyer?.address)}, en adelante ${esT.the_buyer},`;
    addBilingual(
      buyerTextEs,
      (trans, lang) => `${trans.and_other_part}, ${personIdTrans(buyerTitle, buyerName, d.buyer?.nationality, d.buyer?.idType, d.buyer?.idNumber, d.buyer?.nie, trans, lang)} ${trans.with_address} ${v(d.buyer?.address)}, ${trans.hereinafter} ${trans.the_buyer},`,
      200
    );
  }
  ...
  // ========== EXPONEN / DECLARE ==========
  addPara(esT.declare, { bold: true, align: 'center', spacing: 100, color: BRAND.gold, fontSize: 24 });
  if (isMultiLang) {
    langEntries.forEach((entry) => {
      addPara(entry.t.declare, { bold: true, italic: true, align: 'center', spacing: 60, color: LANG_COLORS[entry.lang] });
    });
  }
  ...
  addBilingual(
    `${esT.type_of_property}: ${propType}, ${esT.located_in} ${v(d.property?.address)}, ${esT.registered_in} ${v(d.property?.registry)}, ${esT.property_nr} ${v(d.property?.finca)}, ${esT.volume} ${v(d.property?.tomo)}, ${esT.book} ${v(d.property?.libro)}, ${esT.folio} ${v(d.property?.folio)}, ${esT.cadastral_ref} ${v(d.property?.catastral)}.`,
  ...
  if (d.extraProperties && d.extraProperties.length > 0) {
    d.extraProperties.forEach((prop, idx) => {
      if (prop.enabled) {
        addBilingual(
          `${esT.type_of_property} ${idx + 2}: ${v(prop.type)}, ${esT.cadastral_ref} ${v(prop.catastral)}, ${esT.property_nr} ${v(prop.finca)}, ${esT.volume} ${v(prop.tomo)}, ${esT.book} ${v(prop.libro)}, ${esT.folio} ${v(prop.folio)}.`,
  ...
  // ========== CLÁUSULAS ==========
  addPara(esT.clauses, { bold: true, align: 'center', spacing: 100, color: BRAND.gold, fontSize: 24 });
  if (isMultiLang) {
    langEntries.forEach((entry) => {
      addPara(entry.t.clauses, { bold: true, italic: true, align: 'center', spacing: 60, color: LANG_COLORS[entry.lang] });
    });
  }
  ...
  if (d.type === 'arras') {
    ...
    addBilingual(
      `1. ${esT.clause_1}.- ${sellerFull} ${esT.first_arras} ${buyerFull}, ${esT.who_agrees}`,
      (trans, lang) => `1. ${trans.clause_1}.- ${sellerFull} ${trans.first_arras} ${buyerFull}, ${trans.who_agrees}`,
      200
    );

    // SEGUNDA - Price
    addBilingual(
      `2. ${esT.clause_2}.- ${esT.second_price} ${formatMoney(d.price?.total)} ${esT.plus_taxes}`,
    ...
    // Reservation payment
    addBilingual(
      `a) La cantidad de ${formatMoney(d.price?.reservation)} ${esT.reservation_paid} ${formatDate(d.price?.reservationDate, 'es')} ${esT.through_transfer}`,
      (trans, lang) => `a) ${formatMoney(d.price?.reservation, lang)} ${trans.reservation_paid} ${formatDate(d.price?.reservationDate, lang)} ${trans.through_transfer}`,
      200
    );

    // Arras payment
    addBilingual(
      `b) La cantidad de ${formatMoney(d.price?.arras)} ${esT.arras_to_pay}`,
      (trans, lang) => `b) ${formatMoney(d.price?.arras, lang)} ${trans.arras_to_pay}`,
      200
    );

    // Bank details
    addPara(`IBAN: ${bankIban}`, { bold: true, spacing: 80 });
    addPara(`Banco: ${bankName}`, { spacing: 80 });
    addPara(`Beneficiario: ${bankBeneficiary}`, { spacing: 80 });
    addPara(`${esT.concept_arras}`, { bold: true, spacing: 200 });

    // Arras deadline
    addBilingual(
      `${esT.transfer_effective} ${formatDate(d.price?.arrasDeadline, 'es')} ${esT.or_before}`,
      (trans, lang) => `${trans.transfer_effective} ${formatDate(d.price?.arrasDeadline, lang)} ${trans.or_before}`,
      200
    );

    // Arras penitenciales
    addBilingual(esT.arras_penitenciales, (trans, lang) => trans.arras_penitenciales, 200);

    // Remaining payment
    addBilingual(
      `c) La cantidad de ${formatMoney(d.price?.remaining)} ${esT.remaining_payment}, ${esT.at_signing} ${v(d.notaryLocation)} ${v(d.notary)}, ${esT.on_or_before} ${formatDate(d.price?.notaryDate, 'es')}.`,
      (trans, lang) => `c) ${formatMoney(d.price?.remaining, lang)} ${trans.remaining_payment}, ${trans.at_signing} ${v(d.notaryLocation)} ${v(d.notary)}, ${trans.on_or_before} ${formatDate(d.price?.notaryDate, lang)}.`,
      200
    );
    ...
    addPara(`Banco: ${bankName}`, { spacing: 80 });
    addPara(`Beneficiario: ${bankBeneficiary}`, { spacing: 80 });
    addPara(`${esT.concept_purchase}`, { bold: true, spacing: 200 });
    ...
    addBilingual(esT.res_all_parties, (trans, lang) => trans.res_all_parties, 200);

    // Owners line with ALL names
    const sellerFull = d.seller?.hasPartner && d.seller?.partner?.name
      ? `${sellerTitle} ${sellerName} y ${d.seller.partner.title === 'Dona' || d.seller.partner.title === 'Doña' ? 'Dña.' : 'D.'} ${v(d.seller.partner.name)}`
      : `${sellerTitle} ${sellerName}`;
    const buyerFull = d.buyer?.hasPartner && d.buyer?.partner?.name
    ...
    addBilingual(
      `${sellerFull}, y ${buyerFull} ${esT.res_owners}`,
      (trans, lang) => `${sellerFull}, ${trans.and_partner || 'and'} ${buyerFull} ${trans.res_owners}`,
      200
    );
    ...
    addBilingual(
      `${esT.res_price_clause} ${formatMoney(d.price?.total)}, ${esT.res_furniture_not}, ${esT.res_excl_taxes}`,
      (trans, lang) => `${trans.res_price_clause} ${formatMoney(d.price?.total, lang)}, ${trans.res_furniture_not}, ${trans.res_excl_taxes}`,
      200
    );
    ...
    addBilingual(
      `a) ${formatMoney(d.price?.reservation)} ${esT.res_non_refundable}`,
    ...
    addPara(`Banco: ${bankName}`, { spacing: 80 });
    addPara(`Beneficiario: ${bankBeneficiary}`, { spacing: 80 });
    addPara(`${esT.res_concept} ${formatDate(d.date, 'es')}`, { bold: true, spacing: 200 });

    addBilingual(
      `${esT.res_arras_commit} ${v(d.arrasSignDays)} ${esT.res_calendar_days} ${esT.res_working_days} ${formatMoney(d.price?.arras)} ${esT.res_to_seller} ${esT.res_as_arras}`,
    ...
    addBilingual(
      `${esT.res_max_date} ${formatDate(d.price?.notaryDate, 'es')}.`,
      (trans, lang) => `${trans.res_max_date} ${formatDate(d.price?.notaryDate, lang)}.`,
      200
    );

    addBilingual(esT.res_transfer_free, (trans, lang) => trans.res_transfer_free, 200);
    addBilingual(esT.res_cbli_role, (trans, lang) => trans.res_cbli_role, 200);
        ...
        children: [
          new TableCell({
            ...
            children: [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: `${esT.seller_label} / ${isMultiLang ? langEntries[0].t.seller_label : 'SELLER'}`, bold: true, size: 20, color: BRAND.gold })],
            })],
          }),
          new TableCell({
            ...
            children: [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: `${esT.buyer_label} / ${isMultiLang ? langEntries[0].t.buyer_label : 'BUYER'}`, bold: true, size: 20, color: BRAND.gold })],
            })],
          }),
          new TableCell({
            width: { size: 3009, type: WidthType.DXA },
            ...
            children: [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: 'INTERMEDIARIO', bold: true, size: 20, color: BRAND.gold })],
            })],
          }),
        ],
      }),
      ...
      new TableRow({
        children: [
          new TableCell({
            ...
            ...(d.seller?.hasPartner && d.seller?.partner?.name ? [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: v(d.seller.partner.name), size: 18 })],
            })] : []),
            ],
          }),
          new TableCell({
            ...
            ...(d.buyer?.hasPartner && d.buyer?.partner?.name ? [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: v(d.buyer.partner.name), size: 18 })],
            })] : []),
            ],
          }),
          new TableCell({
