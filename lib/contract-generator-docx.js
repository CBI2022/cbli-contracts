...
export async function generateContractDocx(formData) {
  ...
  const addBilingual = (esText, transTextOrFn, spacingAfter = 200) => {
    ...
    const esRuns = [];
    if (typeof esText === 'string') {
      esRuns.push(new TextRun({ text: esText, bold: true }));
    } else if (Array.isArray(esText)) {
      esText.forEach((seg) => {
        esRuns.push(new TextRun({ text: seg.text, bold: seg.bold !== false, italic: seg.italic || false }));
      });
    }
    ...
    if (hasTranslations) {
      langEntries.forEach((entry, idx) => {
        ...
        const transRuns = [];
        if (typeof transText === 'string') {
          transRuns.push(new TextRun({ text: transText, italic: true, color }));
        } else if (Array.isArray(transText)) {
          transText.forEach((seg) => {
            transRuns.push(new TextRun({ text: seg.text, italic: true, bold: seg.bold || false, color }));
          });
        }
        paragraphs.push(new Paragraph({
          children: transRuns,
