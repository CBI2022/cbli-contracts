...
const translations = {
  ...
  en: {
    ...
    comm_has_offered: "That COSTA BLANCA LUXURY INVESTMENTS S.L. has offered and visited with the client",
...
export async function generateContractDocx(formData) {
  ...
  if (d.type === 'arras') {
    ...
    if (selHasPartner) {
      sellerSegsEs.push({ text: ' y ' }, { text: `${selTitle2} ${v(d.seller.partner.name)}`, bold: true });
    }
    sellerSegsEs.push(
      { text: `, de nacionalidad ` }, { text: v(d.seller?.nationality), bold: true },
      { text: selHasPartner ? ', mayores de edad, casados, con ' : ', mayor de edad, con ' },
      { text: selIdType1 }, { text: ' número ' }, { text: selIdNum1, bold: true },
    );
    if (selHasPartner) {
      sellerSegsEs.push({ text: ' y ' }, { text: selIdNum2, bold: true });
    }
    sellerSegsEs.push(
      { text: ', con domicilio en ' }, { text: v(d.seller?.address), bold: true },
      { text: '. En adelante ' }, { text: 'el Vendedor.', bold: true },
    );
    addBilingual(sellerSegsEs, (trans, lang) => {
      const mrs = (d.seller?.title === 'Doña' || d.seller?.title === 'Dona') ? 'Mrs' : 'Mr';
      const mrs2 = selHasPartner ? ((d.seller.partner.title === 'Doña' || d.seller.partner.title === 'Dona') ? 'Mrs' : 'Mr') : '';
      const natTranslated = NATIONALITY_TRANSLATIONS[lang]?.[v(d.seller?.nationality).toLowerCase()] || v(d.seller?.nationality);
      let text = `${trans.comm_on_one_hand} ${mrs} ${v(sellerName)}`;
      if (selHasPartner) text += ` ${trans.comm_and} ${mrs2} ${v(d.seller.partner.name)}`;
      text += `, ${trans.comm_of_nationality} ${natTranslated}, ${trans.comm_of_legal_age}`;
      if (selHasPartner) text += `, ${trans.comm_married}`;
      text += `, ${trans.comm_with} ${selIdType1} ${trans.comm_number} ${selIdNum1}`;
      if (selHasPartner) text += ` ${trans.comm_and} ${selIdNum2}`;
      text += `, ${trans.comm_residing_at} ${v(d.seller?.address)}. ${trans.comm_hereinafter_seller}`;
      return text;
    }, 200);
    ...
    const dicenSegsEs = [
      ...
      { text: ' ha ofrecido y visitado con la cliente ' },
