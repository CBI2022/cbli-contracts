...
export async function generateContractDocx(formData) {
  ...
  // Contract title (Spanish + translations)
  const titleEs = d.type === 'arras' ? esT.arras_title : esT.reservation_title;
  addPara(titleEs, { bold: true, align: 'center', fontSize: 28, spacing: 100, color: BRAND.gold });

  if (isMultiLang) {
    langEntries.forEach((entry) => {
      const titleTrans = d.type === 'arras' ? entry.t.arras_title : entry.t.reservation_title;
      addPara(titleTrans, { bold: true, italic: true, align: 'center', fontSize: 24, spacing: 60, color: LANG_COLORS[entry.lang] });
    });
  }
  ...
  const sellerTextEs = `${esT.on_one_part}, ${sellerTitle} ${sellerName}, ${esT.of_legal_age}, ${esT.of_nationality} ${v(d.seller?.nationality)}, ${esT.with_valid} ${v(d.seller?.idType)} ${esT.number} ${v(d.seller?.idNumber)} ${esT.and_nie} ${v(d.seller?.nie || '___')}, ${esT.with_address} ${v(d.seller?.address)}, ${esT.hereinafter} ${esT.the_seller},`;

  addBilingual(
    sellerTextEs,
    (trans, lang) => `${trans.on_one_part}, ${sellerTitle} ${sellerName}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(d.seller?.nationality, lang)}, ${trans.with_valid} ${v(d.seller?.idType)} ${trans.number} ${v(d.seller?.idNumber)} ${trans.and_nie} ${v(d.seller?.nie || '___')}, ${trans.with_address} ${v(d.seller?.address)}, ${trans.hereinafter} ${trans.the_seller},`,
    200
  );

  // Seller partner (if applicable)
  if (d.seller?.hasPartner && d.seller?.partner?.name) {
    const sp = d.seller.partner;
    const spTitle = sp.title === 'Don' ? 'D.' : 'Dña.';
    const spTextEs = `${esT.and_partner} ${spTitle} ${v(sp.name)}, ${esT.of_legal_age}, ${esT.of_nationality} ${v(sp.nationality)}, ${esT.with_valid} ${v(sp.idType)} ${esT.number} ${v(sp.idNumber)} ${esT.and_nie} ${v(sp.nie || '___')},`;
    addBilingual(
      spTextEs,
      (trans, lang) => `${trans.and_partner} ${spTitle} ${v(sp.name)}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(sp.nationality, lang)}, ${trans.with_valid} ${v(sp.idType)} ${trans.number} ${v(sp.idNumber)} ${trans.and_nie} ${v(sp.nie || '___')},`,
      200
    );
  }
  ...
  const buyerTextEs = `${esT.and_other_part}, ${buyerTitle} ${buyerName}, ${esT.of_legal_age}, ${esT.of_nationality} ${v(d.buyer?.nationality)}, ${esT.with_valid} ${v(d.buyer?.idType)} ${esT.number} ${v(d.buyer?.idNumber)} ${esT.and_nie} ${v(d.buyer?.nie || '___')}, ${esT.with_address} ${v(d.buyer?.address)}, ${esT.hereinafter} ${esT.the_buyer},`;

  addBilingual(
    buyerTextEs,
    (trans, lang) => `${trans.and_other_part}, ${buyerTitle} ${buyerName}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(d.buyer?.nationality, lang)}, ${trans.with_valid} ${v(d.buyer?.idType)} ${trans.number} ${v(d.buyer?.idNumber)} ${trans.and_nie} ${v(d.buyer?.nie || '___')}, ${trans.with_address} ${v(d.buyer?.address)}, ${trans.hereinafter} ${trans.the_buyer},`,
    200
  );

  // Buyer partner (if applicable)
  if (d.buyer?.hasPartner && d.buyer?.partner?.name) {
    const bp = d.buyer.partner;
    const bpTitle = bp.title === 'Don' ? 'D.' : 'Dña.';
    const bpTextEs = `${esT.and_partner} ${bpTitle} ${v(bp.name)}, ${esT.of_legal_age}, ${esT.of_nationality} ${v(bp.nationality)}, ${esT.with_valid} ${v(bp.idType)} ${esT.number} ${v(bp.idNumber)} ${esT.and_nie} ${v(bp.nie || '___')},`;
    addBilingual(
      bpTextEs,
      (trans, lang) => `${trans.and_partner} ${bpTitle} ${v(bp.name)}, ${trans.of_legal_age}, ${trans.of_nationality} ${translateNat(bp.nationality, lang)}, ${trans.with_valid} ${v(bp.idType)} ${trans.number} ${v(bp.idNumber)} ${trans.and_nie} ${v(bp.nie || '___')},`,
      200
    );
  }
