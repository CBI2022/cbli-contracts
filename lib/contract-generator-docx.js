...
export async function generateContractDocx(formData) {
  ...
  if (d.type === 'arras') {
    ...
    addBilingual(sellerSegsEs, (trans, lang) => {
      ...
      let text = `On the one hand, ${mrs} ${v(sellerName)}`;
      if (selHasPartner) text += ` and ${mrs2} ${v(d.seller.partner.name)}`;
      text += `, of ${v(d.seller?.nationality)} nationality, of legal age`;
      if (selHasPartner) text += ', married';
      text += `, with ${selIdType1} number ${selIdNum1}`;
      if (selHasPartner) text += ` and ${selIdNum2}`;
      text += `, residing at ${v(d.seller?.address)}. Hereinafter the Seller.`;
      return text;
    }, 200);
    ...
    addBilingual(
      ...
      (trans, lang) => 'And on the other hand, the company COSTA BLANCA LUXURY INVESTMENTS S.L. with CIF B56281082, with address at Puerto Deportivo Campomanes 59, 03590, Altea, Alicante on behalf of Don BRUNO FELIPE.',
      200
    );
    ...
    addBilingual(dicenSegsEs, (trans, lang) => {
      let text = `That COSTA BLANCA LUXURY INVESTMENTS S.L. has offered and visited with the client ${buyTitle1} ${v(buyerName)}, of legal age, with nationality ${v(d.buyer?.nationality)}, provided with ${v(d.buyer?.idType)} number: ${v(d.buyer?.idNumber)}`;
      if (buyHasPartner) {
        text += `, and ${buyTitle2} ${v(d.buyer.partner.name)}, of legal age, with nationality ${v(d.buyer?.partner?.nationality || d.buyer?.nationality)}, provided with ${v(d.buyer.partner.idType)} number: ${v(d.buyer.partner.idNumber)}`;
      }
      text += `, the following property of the Seller: URBAN: ${v(d.property?.type)}, located at ${v(d.property?.address)}`;
      if (hasVal(d.property?.registry)) text += ` registered in the ${d.property.registry.trim()} Property Registry`;
      if (hasVal(d.property?.finca)) text += `, property no. ${d.property.finca.trim()}`;
      if (hasVal(d.property?.tomo)) text += `, volume ${d.property.tomo.trim()}`;
      if (hasVal(d.property?.libro)) text += `, book ${d.property.libro.trim()}`;
      if (hasVal(d.property?.folio)) text += `, folio ${d.property.folio.trim()}`;
      if (hasVal(d.property?.catastral)) text += `, with only cadastral reference: ${d.property.catastral.trim()}`;
      text += '.';
      return text;
    }, 200);
    ...
    addBilingual(
      ...
      (trans, lang) => [
        { text: 'The clients mentioned above want to buy through the Real Estate agency ' },
        { text: 'COSTA BLANCA LUXURY INVESTMENTS S.L.', bold: true },
        { text: ' the property described in the previous section.' },
      ],
      200
    );
    ...
    addBilingual(
      ...
      (trans, lang) => [
        { text: `The agreed price for the sale of said home is ` },
        { text: formatMoneyFull(d.price?.total, lang), bold: true },
        { text: '. The agreed commission of the intermediary is included in the sale price of a total of ' },
        { text: formatMoneyFull(commAfterDiscount > 0 ? String(commAfterDiscount) : d.commission?.totalCommission, lang), bold: true },
        { text: ' + 21% VAT = a total of ' },
    ...
    addBilingual(
      ...
      (trans, lang) => [
        { text: `Upon the payment of the 10% deposit as downpayment money into the seller's account, the seller will then pay a part of the total agreed commission, which amounts to ` },
    ...
    addBilingual(
      ...
      (trans, lang) => [
        { text: 'The remaining commission, which amounts to ' },
        ...
        { text: 'COSTA BLANCA LUXURY INVESTMENTS SL', bold: true },
        { text: ' either before or on the same day as the signing at the agreed-upon notary.' },
      ],
      200
    );
    ...
    addBilingual(
      ...
      (trans, lang) => 'Both parties read this document, find it in accordance with their will, ratify its content, and sign it in duplicate, for a single purpose, at the aforementioned place and date.',
      300
    );
