...
export async function generateContractDocx(formData) {
  ...
  // Property details — form data in bold, skip empty/zero optional fields
  const propType = v(d.property?.type);
  const hasVal = (val) => val && val.trim() && val.trim() !== '0' && val.trim() !== '___';
  const buildPropSegs = (t) => {
    const segs = [
      { text: `${t.type_of_property}: ` }, { text: propType, bold: true },
      { text: `, ${t.located_in} ` }, { text: v(d.property?.address), bold: true },
    ];
    if (hasVal(d.property?.registry)) segs.push({ text: `, ${t.registered_in} ` }, { text: d.property.registry.trim(), bold: true });
    if (hasVal(d.property?.finca)) segs.push({ text: `, ${t.property_nr} ` }, { text: d.property.finca.trim(), bold: true });
    if (hasVal(d.property?.tomo)) segs.push({ text: `, ${t.volume} ` }, { text: d.property.tomo.trim(), bold: true });
    if (hasVal(d.property?.libro)) segs.push({ text: `, ${t.book} ` }, { text: d.property.libro.trim(), bold: true });
    if (hasVal(d.property?.folio)) segs.push({ text: `, ${t.folio} ` }, { text: d.property.folio.trim(), bold: true });
    if (hasVal(d.property?.catastral)) segs.push({ text: `, ${t.cadastral_ref} ` }, { text: d.property.catastral.trim(), bold: true });
    segs.push({ text: '.' });
    return segs;
  };
  addBilingual(
    buildPropSegs(esT),
    (trans, lang) => buildPropSegs(trans),
    200
  );

  // Extra properties — form data in bold, skip empty/zero optional fields
  if (d.extraProperties && d.extraProperties.length > 0) {
    d.extraProperties.forEach((prop, idx) => {
      if (prop.enabled) {
        const buildExtraSegs = (t) => {
          const segs = [{ text: `${t.type_of_property} ${idx + 2}: ` }, { text: v(prop.type), bold: true }];
          if (hasVal(prop.catastral)) segs.push({ text: `, ${t.cadastral_ref} ` }, { text: prop.catastral.trim(), bold: true });
          if (hasVal(prop.finca)) segs.push({ text: `, ${t.property_nr} ` }, { text: prop.finca.trim(), bold: true });
          if (hasVal(prop.tomo)) segs.push({ text: `, ${t.volume} ` }, { text: prop.tomo.trim(), bold: true });
          if (hasVal(prop.libro)) segs.push({ text: `, ${t.book} ` }, { text: prop.libro.trim(), bold: true });
          if (hasVal(prop.folio)) segs.push({ text: `, ${t.folio} ` }, { text: prop.folio.trim(), bold: true });
          segs.push({ text: '.' });
          return segs;
        };
        addBilingual(buildExtraSegs(esT), (trans, lang) => buildExtraSegs(trans), 200);
      }
    });
  }
