...
export async function generateContractDocx(formData) {
  ...
  addBilingual(buildPropSegs(), null, 200);

  // Extra properties — form data in bold, skip empty optional fields
  if (d.extraProperties && d.extraProperties.length > 0) {
    d.extraProperties.forEach((prop, idx) => {
      if (prop.enabled) {
        const buildExtraSegs = () => {
          const segs = [{ text: v(prop.type), bold: true }];
        ...
        addBilingual(buildExtraSegs(), null, 200);
      }
    });
  }
