...
export async function generateContractDocx(formData) {
  ...
  const addPara = (text, opts = {}) => {
    const { bold = false, italic = false, align = 'left', fontSize = FONT_SIZE, spacing = 200, color = null, underline = false } = opts;
  ...
  if (d.type === 'arras') {
    ...
    // Owners line — only seller name(s) as property owner
    const sellerFull = d.seller?.hasPartner && d.seller?.partner?.name
      ? `${sellerTitle} ${sellerName} y ${d.seller.partner.title === 'Dona' || d.seller.partner.title === 'Doña' ? 'Dña.' : 'D.'} ${v(d.seller.partner.name)}`
      : `${sellerTitle} ${sellerName}`;

    addBilingual(
      `${sellerFull} ${esT.res_owners}`,
      (trans, lang) => `${sellerFull} ${trans.res_owners}`,
      200
    );
    ...
    addBilingual(
      `2. ${esT.clause_2}.- ${esT.res_price_clause} ${formatMoney(d.price?.total)}, ${esT.res_furniture_not}, ${esT.res_excl_taxes}`,
      (trans, lang) => `2. ${trans.clause_2}.- ${trans.res_price_clause} ${formatMoney(d.price?.total, lang)}, ${trans.res_furniture_not}, ${trans.res_excl_taxes}`,
      200
    );
    ...
    addBilingual(
      `3. ${esT.clause_3}.- ${esT.res_arras_commit} ${v(d.arrasSignDays)} ${esT.res_calendar_days} ${esT.res_working_days} ${formatMoney(d.price?.arras)} ${esT.res_to_seller} ${esT.res_as_arras}`,
      (trans, lang) => `3. ${trans.clause_3}.- ${trans.res_arras_commit} ${v(d.arrasSignDays)} ${trans.res_calendar_days} ${trans.res_working_days} ${formatMoney(d.price?.arras, lang)} ${trans.res_to_seller} ${trans.res_as_arras}`,
      200
    );

    // 4. CUARTA — Max date
    addBilingual(
      `4. ${esT.clause_4}.- ${esT.res_max_date} ${formatDate(d.price?.notaryDate, 'es')}.`,
      (trans, lang) => `4. ${trans.clause_4}.- ${trans.res_max_date} ${formatDate(d.price?.notaryDate, lang)}.`,
      200
    );
